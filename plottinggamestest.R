...
game_number <- as.integer(game_number)
...
#loads xGmodel
xGModel <-load('~/DataAnalysis/Programming Stuff/R/
               Rprojects/xGmodel/xGmodelver2.rda')
...
#calculates the running sum for the step graphs 
...
#to step plot them
xg_graph_df <- gather(pbp_df, 'run_home_xg', 'run_away_xg', 
...
xG_plot  <- ggplot(aes(x = game_seconds/60, y = running_xg), 
            data = xg_graph_df) +
            ...
            scale_color_manual(labels = c(home_team, away_team), 
                values = c("red", "blue")) +
...
fenwick_pbp <- filter(pbp_df, event_type %in% c("SHOT", "MISS", "GOAL"))
groupd_player_xg <- pbp_df %>% group_by(event_player_1, event_team) %>%
...
dir.create(paste('~/Desktop/HockeyStuff/xGGameBreakdowns/2018/', game_number))
setwd(paste('~/Desktop/HockeyStuff/xGGameBreakdowns/2018/', game_number))
ggsave('RunningxG.png', plot = xG_plot)
ggsave('xGlocations.png', plot = xg_locations_plot)
